        ...
        for examp_index in np.arange(count):
            ...
            if not is_test:
                ...
                for _ in range(batch_size):
                    ...
                    seq_sel = the_sequence[
                              idx + (source_seq_len - input_n):(idx + source_seq_len + output_n), :]
                    seq_sel = np.pand_dims(seq_sel, axis=0)
                    if len(sampled_seq) == 0:
                        sampled_seq = seq_sel
                        complete_seq = the_sequence
                    else:
...
def rotmat2euler_torch(R, dev='cuda:0'):
    """
    Converts a rotation matrix to euler angles
    ...
    eul = torch.zeros(n, 3).float().to(dev)
    ...
    if len(idx_spec1) > 0:
        ...
        eul_spec1 = torch.zeros(len(idx_spec1), 3).float().to(dev)
        eul_spec1[:, 2] = 0
        eul_spec1[:, 1] = -np.pi / 2
    ...
    if len(idx_spec2) > 0:
        ...
        eul_spec2 = torch.zeros(len(idx_spec2), 3).float().to(dev)
        eul_spec2[:, 2] = 0
        eul_spec2[:, 1] = np.pi / 2
    ...
    if len(idx_remain) > 0:
        ...
        eul_remain = torch.zeros(len(idx_remain), 3).float().to(dev)
...
def expmap2rotmat_torch(r, dev='cuda:0'):
    """
...
def expmap2xyz_torch(expmap, dev='cuda:0'):
    """
